...
#include <psapi.h>
  ...
  #endif
#endif
#if defined(HX_LINUX) || defined(HX_MACOS) || defined(HX_CYGWIN) || defined(HX_ANDROID) || defined(HX_TIZEN) || defined(HX_IOS)
#include <unistd.h>
#endif
...
static int GetMaxHardwareThreads()
{
   int count = 1;
   #ifdef HX_WINDOWS
      SYSTEM_INFO sysinfo;
   ...
   #elif defined(HX_LINUX) || defined(HX_MACOS) || defined(HX_CYGWIN) || defined(HX_ANDROID) || defined(HX_TIZEN)
...
{
   sRunningThreads |= ((uint64_t)1<<inThreadId);
...
struct GlobalChunks
{
   ...
   MarkChunk *pushJob(MarkChunk *inChunk,bool inAndAlloc)
   {
      ...
      if (MAX_GC_THREADS>1 && sLazyThreads)
      {
         ...
         for(int tid=0; tid<sgThreadCount && tid<MAX_GC_THREADS; tid++)
         {
            if (!(sRunningThreads & ((uint64_t)1<<tid)))
            {
   ...
   {
      if (!(sRunningThreads & ((uint64_t)1<<inThreadId)))
      {
   ...
   MarkChunk *popJobOrFinish(MarkChunk *inChunk,int inThreadId)
   {
      ...
      if (sAllThreads)
      {
         ...
         if (!result)
         {
            ...
            {
               if ( sgThreadPoolAbort || sAllThreads == ((uint64_t)1<<inThreadId) )
                  break;
               if (processList)
               {
...
// Forward declare LocalAllocator before GlobalAllocator
class LocalAllocator;
...
public:
   ...
   void AddLocal(LocalAllocator *inAlloc)
   {
      ...
      mLocalAllocs.push((LocalAllocator *)inAlloc);
   ...
   bool ReturnToPoolLocked(LocalAllocator *inAlloc)
   {
      ...
      for(int p=0;p<LOCAL_POOL_SIZE;p++)
      {
         if (!mLocalPool[p])
         {
            mLocalPool[p] = (LocalAllocator *)inAlloc;
            return true;
         }
      }
   ...
   LocalAllocator *GetPooledAllocator()
   {
      ...
      for(int p=0;p<LOCAL_POOL_SIZE;p++)
      {
         if (mLocalPool[p])
         {
            LocalAllocator *result = (LocalAllocator *)mLocalPool[p];
            mLocalPool[p] = 0;
            return result;
         }
      }
   ...
   void RemoveLocalLocked(LocalAllocator *inAlloc)
   {
      ...
      // Use index loop instead of qerase_val to avoid template matching issues
      for(int i=0; i<mLocalAllocs.size(); i++)
      {
         if (mLocalAllocs[i] == inAlloc)
         {
            mLocalAllocs.qerase(i);
            return;
         }
      }
  ...
  void MoveSpecial(hx::Object *inTo, hx::Object *inFrom, int size)
   ...
   {
      ThreadPoolAutoLock l(sThreadPoolLock);
      if (sRunningThreads & ((uint64_t)1<<inId))
      {
         sRunningThreads &= ~((uint64_t)1<<inId);
         sLazyThreads = sRunningThreads != sAllThreads;
   ...
   void waitForThreadWake(int inId)
   {
      #ifdef HX_GC_PTHREADS
      {
         ...
         // Spurious wake?
         while( !(sRunningThreads & ((uint64_t)1<<inId) ) )
   ...
   void StartThreadJobs(ThreadPoolJob inJob, int inWorkers, bool inWait, int inThreadLimit = -1)
   {
      ...
      int hwThreads = GetMaxHardwareThreads();
      int maxUseThreads = std::min((int)MAX_GC_THREADS, hwThreads);
      ...
      sgThreadCount = inThreadLimit<0 ? maxUseThreads : std::min(maxUseThreads, inThreadLimit) ;

      int start = std::min(inWorkers, sgThreadCount );

      sAllThreads = (sgThreadCount >= 64) ? ~((uint64_t)0) : (((uint64_t)1<<sgThreadCount) - 1);

      sRunningThreads = (start >= 64) ? ~((uint64_t)0) : (((uint64_t)1<<start) - 1);
      ...
      if (inWait)
      {
         ...
         if (sRunningThreads)
         {
            printf("Bad thread stop %lld\n", (long long)sRunningThreads);
            DebuggerTrap();
         }
      }
   }
   ...
   void Collect(bool inMajor, bool inForceCompact, bool inLocked,bool inFreeIsFragged)
   {
      ...
      */
      ...
      if (oldRowsInUse > 0)
         mSurvivalRate = (double)mRowsInUse / oldRowsInUse;
      else
         mSurvivalRate = 0.5;
...
public:
   ...
   int             mStackLocks;
   size_t          mCommittedBytes;

   size_t GetAllocatedSinceGC()
   {
       size_t total = mCommittedBytes;
       if (allocBase)
       {
           int prevHole = mCurrentHole - 1;
           if (prevHole >= 0)
           {
               #ifdef HXCPP_GC_NURSERY
               unsigned char *start = allocBase + mCurrentRange[prevHole].start + sizeof(int);
               total += (spaceFirst - start);
               #else
               int start = mCurrentRange[prevHole].start;
               total += (spaceStart - start);
               #endif
           }
       }
       return total;
         ...
         if (mMoreHoles)
         {
            if (allocBase)
            {
               ...
               if (prevHole >= 0)
               {
                  #ifdef HXCPP_GC_NURSERY
                  unsigned char *start = allocBase + mCurrentRange[prevHole].start + sizeof(int);
                  mCommittedBytes += (spaceFirst - start);
                  #else
                  int start = mCurrentRange[prevHole].start;
                  mCommittedBytes += (spaceStart - start);
                  #endif
               }
            }
...
void SetTopOfStack(int *inTop,bool inForce)
{
   ...
   if (inTop)
   {
      ...
      {
         if (hx::tlsStackContext==0)
   {
      hx::GCPrepareMultiThreaded();
      hx::RegisterCurrentThread(inTop);
      threadAttached = true;
   }
      }
   }

   LocalAllocator *tla = (LocalAllocator *)(hx::ImmixAllocator *)hx::tlsStackContext;
...
void GlobalAllocator::ResetCommittedBytes()
{
   for(int i=0;i<mLocalAllocs.size();i++)
   {
      if (mLocalAllocs[i])
         mLocalAllocs[i]->mCommittedBytes = 0;
   }
}
...
static int ReadEnvInt(const char *inName, int inDefault)
{
   const char *val = getenv(inName);
   if (val)
      return atoi(val);
   return inDefault;
}
